    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            Action::NavigateToEpicDetail { epic_id } => {
                let epic_detail_page = EpicDetail { db: self.db.clone(), epic_id }; 
                self.pages.push(Box::new(epic_detail_page));
            }
            Action::NavigateToStoryDetail { epic_id, story_id } => {
                let story_detail_page = StoryDetail { db: self.db.clone(), epic_id, story_id };
                self.pages.push(Box::new(story_detail_page));
            }
            ...
            Action::CreateEpic => {
                println!("Creating epic");
                let epic = (self.prompts.create_epic)();
                self.db.create_epic(epic).with_context(|| "failed to create epic")?;
            }
            Action::UpdateEpicStatus { epic_id } => {
                let status = (self.prompts.update_status)().with_context(|| "failed to get status")?;
                self.db
                    .update_epic_status(epic_id, status)
                    .map_err(|_| anyhow!("failed to update epic status"))?;
            }
            Action::DeleteEpic { epic_id } => {
                if (self.prompts.delete_epic)() {
                    self.db.delete_epic(epic_id).with_context(|| "failed to delete epic")?;
                }
                if !self.pages.is_empty() {
                    self.pages.pop();
                }
            ...
            Action::CreateStory { epic_id } => {
                ...
                self.db.create_story(story, epic_id).with_context(|| "failed to create story")?;
            }
            Action::UpdateStoryStatus { story_id } => {
                let status = (self.prompts.update_status)().with_context(|| "failed to get status")?;
                self.db
                    .update_story_status(story_id, status)
                    .map_err(|_| anyhow!("failed to update story status"))?;
            }
            Action::DeleteStory { epic_id, story_id } => {
                if (self.prompts.delete_story)() {
                    self.db.delete_story(epic_id, story_id).with_context(|| "failed to delete story")?;
                    if !self.pages.is_empty() {
                        self.pages.pop();
                    }
                }
